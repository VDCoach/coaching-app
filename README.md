...
* **🏋️ Séance :** Exercices en accordéon, **chronomètre de repos** (lancé automatiquement quand tu coches une série). **Supersets** (2 exercices côte à côte, 3+ en colonne) avec repos global uniquement à la fin du dernier exercice. Auto-scroll fluide vers le prochain exercice / bloc superset à la dernière série cochée.
...
* **⏱️ Chronos :** Repos (lancé au coche d'une série) et effort (compte à rebours) ; clic sur le chrono pour mettre en pause / reprendre. Bouton × pour arrêter le repos. Réglage optionnel dans les Paramètres pour activer des **bips plus forts** sur les 3 dernières secondes.
